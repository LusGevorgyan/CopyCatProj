...
const PricingFormPro = () => {
    const [formPro, setFormPro] = useState(dataFormPro)

    const onUpdate = (itemId) => {
        const wrapper = formPro.map(({id, isUpdate, text}) => {
        ...
        <div className='mx-[10px] pt-[30px] px-[40px] pb-[10px]'>
            ...
            <div className='text-[16px] text-center mb-[20px] leading-[24px]'> Best for individual coders </div>
            {
                formPro.map(({id, isUpdate, text}) => (
                    <div 
                        className='cursor-pointer flex items-center justify-start mb-[10px]' 
                        key={id}
                        style={{
